...
If you've adding a goal entry incorrectly, try using the `~goal`, `~g` or `editgoal` command to the goals you have. +
...
If you've added a specific goal that you're unlikely to complete and want to remove it from CollegeZone, try using the `-goal`, `-g` or `deletegoal` command. +
*_Formats:_*
```java
-goal INDEX
```
