...
==== Seeking the Resident Assistant of a student : `seek` [Since v1.3]
Seek the Resident Assistant (RA) of the student whose name contains any of the given keywords. +
*_Formats:_*
```java
seek NAME`
```
...
* Only full words will be matched e.g. `Han` will not match `Hans`
* Resident Assistant (RA) of the student's name matching at least one keyword will be returned (i.e. `OR` search). e.g. `Hans Bo` will return `Hans Gruber's RA`, `Bo Yang's RA`
****
...
You can set a reminder into the calendar in _CollegeZone_ +
...
Edits an existing reminder in _CollegeZone_. +
...
You can delete a reminder that you have set into the calendar in _CollegeZone_ +
